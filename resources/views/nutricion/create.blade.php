<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">
            {{ __('Registrar Información Nutricional') }}
        </h2>
    </x-slot>
                    ...
                    <form action="{{ route('nutricion.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="informacion" 
                                      ...
                                      placeholder="Preferencias, alergias, etc.">{{ old('informacion') }}</textarea>
                            @error('informacion')
                                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="plan_dieta" 
                                      ...
                                      placeholder="Plan de dieta asignado">{{ old('plan_dieta') }}</textarea>
                            @error('plan_dieta')
                                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
